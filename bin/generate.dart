...
main(List<String> args) async {
  if (_isHelpCommand(args)) {
    ...
    final platformGenerator = platformGeneratorFactory(configOption.platform);
    if (null == platformGenerator) {
      printError('platform [${configOption.platform}] is not supported');
    } else {
      final errorMessage = platformGenerator.validArguments(configOption);
...
ArgParser _generateArgParser(ConfigOption? config) {
  ...
  parser.addOption('platform',
      ...
      help:
          "[Required] The platform stores all localization strings. Suppoted platforms: 'airTable'");

  parser.addOption('input',
      callback: (String? x) => config?.input = x, help: '[Required] The source of the strings');
  ...
  parser.addOption('api-key',
      callback: (String? x) => config?.apiKey = x, help: '[Optional] Usage of platform specific');
